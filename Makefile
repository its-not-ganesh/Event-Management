...
# Custom base image settings
BASE_IMAGE ?= event-mgmt-base
BASE_TAG ?= latest
...
.PHONY: base-build
base-build: ## Build the custom base image locally
	$(info Building base image $(BASE_IMAGE):$(BASE_TAG)...)
	docker build --rm --tag $(BASE_IMAGE):$(BASE_TAG) docker/base/

.PHONY: base-build-multi
base-build-multi: ## Build multi-arch base image with buildx
	$(info Building multi-arch base image $(BASE_IMAGE):$(BASE_TAG)...)
	docker buildx build --platform linux/amd64,linux/arm64 \
	  --tag $(BASE_IMAGE):$(BASE_TAG) docker/base/ --push
